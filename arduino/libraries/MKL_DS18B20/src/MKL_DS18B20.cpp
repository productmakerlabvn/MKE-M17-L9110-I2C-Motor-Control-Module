//
//    FILE: MKL_DS18B20.cpp
//  AUTHOR: Rob.Tillaart
// VERSION: 0.2.2
//    DATE: 2017-07-25
// PURPOSE: library for MKL_DS18B20 temperature sensor with minimal footprint
//     URL: https://github.com/RobTillaart/MKL_DS18B20_RT
//          https://github.com/RobTillaart/MKL_DS18B20_INT
...
MKL_DS18B20::MKL_DS18B20(OneWire* ow, uint8_t resolution)
{
  _oneWire      = ow;
  _addressFound = false;
  _resolution   = resolution;
  _config       = MKL_DS18B20_CLEAR;
}


bool MKL_DS18B20::begin(uint8_t retries)
{
  _config = MKL_DS18B20_CLEAR;
...
bool MKL_DS18B20::isConnected(uint8_t retries)
{
  _addressFound = false;
...
void MKL_DS18B20::requestTemperatures(void)
{
  _oneWire->reset();
  _oneWire->skip();
...
bool MKL_DS18B20::isConversionComplete(void)
{
  return (_oneWire->read_bit() == 1);
}


float MKL_DS18B20::getTempC(bool checkConnect)
{
...
bool MKL_DS18B20::getAddress(uint8_t* buf)
{
  if (_addressFound)
  {
...
bool MKL_DS18B20::setResolution(uint8_t resolution)
{
  if (isConnected())
  {
    _resolution = resolution;
...
uint8_t MKL_DS18B20::getResolution()
{
  return _resolution;
}


void MKL_DS18B20::setConfig(uint8_t config)
{
  _config = config;
}


uint8_t MKL_DS18B20::getConfig()
{
  return _config;
}
...
//  PRIVATE
//
void MKL_DS18B20::readScratchPad(uint8_t *scratchPad, uint8_t fields)
{
  _oneWire->reset();
...
void MKL_DS18B20::_setResolution()
{
  uint8_t res;
  switch (_resolution)
  {
